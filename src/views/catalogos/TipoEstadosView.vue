  ...
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
      <div class="mb-4 flex flex-col sm:flex-row justify-between items-center bg-gray-50 dark:bg-gray-700/50 p-4 rounded-lg border border-gray-100 dark:border-gray-700">
        <h2 class="text-lg font-semibold text-gray-700 dark:text-gray-300 mb-2 sm:mb-0">Listado de Estados</h2>
        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tr v-if="!loading && items.length === 0">
                <td colspan="4" class="px-6 py-4 text-center text-gray-500">No hay registros creados.</td>
            </tr>
            <tr v-for="item in items" :key="item.id" class="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm">
                  ...
                  <button 
                    ...
                    :class="item.activo ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-600'"
                  >
                    <span class="sr-only">Usar ajuste</span>
                    <span 
                        aria-hidden="true" 
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900 bg-opacity-75">
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-sm flex flex-col">
            <div class="px-6 py-4 border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-800 rounded-t-lg">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ isEditing ? 'Editar Tipo' : 'Nuevo Tipo' }}</h3>
            </div>
            ...
            <div class="p-6">
                ...
                <!-- Estado Checkbox (Opcional en edición rápida) -->
                <div class="flex items-center">
                    ...
                    >
                    <label for="activo-check" class="ml-2 block text-sm text-gray-900 dark:text-gray-300">
                        Activo inicialmente
                    </label>
                </div>
            </div>
...
const fetchItems = async () => {
  loading.value = true;
  try {
    const { data } = await api.get('/tipo-estados');
    items.value = data;
  } catch (error) {
    console.error(error);
  } finally {
...
const editItem = (item: any) => {
  ...
  form.value = { nombre: item.nombre, activo: !!item.activo }; // Ensure boolean
  showModal.value = true;
  nextTick(() => nameInput.value?.focus());
};
...
const save = async () => {
  ...
  try {
    if (isEditing.value && editingId.value) {
      await api.put(`/tipo-estados/${editingId.value}`, form.value);
    } else {
      await api.post('/tipo-estados', form.value);
    }
    closeModal();
    fetchItems();
...
const toggleStatus = async (item: any) => {
    try {
        ...
        await api.put(`/tipo-estados/${item.id}`, { 
            nombre: item.nombre, 
            activo: newState 
        });
...
const deleteItem = async (id: number) => {
  ...
  if (result.isConfirmed) {
    try {
      await api.delete(`/tipo-estados/${id}`);
      fetchItems();
