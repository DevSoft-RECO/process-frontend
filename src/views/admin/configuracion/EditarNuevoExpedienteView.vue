      ...
      <div class="p-6">
        <form @submit.prevent="updateExpediente" class="space-y-6">
          ...
          <div class="flex justify-between items-center pt-6 border-t border-gray-100 dark:border-gray-700">
             <button type="button" @click="deleteExpediente" :disabled="deleting || saving" class="px-6 py-2 bg-red-500 text-white rounded-lg hover:bg-red-700 transition font-bold shadow-md disabled:opacity-50 flex items-center gap-2">
...
const deleteExpediente = async () => {
  const { value: confirmWord } = await Swal.fire({
    title: '¿Estás seguro?',
    text: 'Esta acción eliminará el expediente permanentemente. Para confirmar, escribe la palabra confirmar:',
    input: 'text',
    inputPlaceholder: 'escriba confirmar',
    ...
    inputValidator: (value) => {
      if (!value) {
        return '¡Debes escribir la palabra confirmar!'
      }
  ...
  if (confirmWord && confirmWord.toLowerCase() === 'confirmar') {
    deleting.value = true
    try {
      await api.delete(`/nuevo-expediente-edicion/${selectedExpediente.value.id}`)
      Swal.fire('¡Eliminado!', 'El expediente ha sido borrado correctamente.', 'success')
      resetSearch()
    } catch (error: any) {
      console.error(error)
      const msg = error.response?.data?.message || 'Error al eliminar el expediente'
      Swal.fire('Error', msg, 'error')
    } finally {
