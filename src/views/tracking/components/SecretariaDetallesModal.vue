...
<button v-if="currentState >= 3 && !numeroContrato" 
        @click="handleAction('adjuntar-contrato')" 
        class="px-5 py-2.5 text-white bg-sky-600 rounded-lg hover:bg-sky-700 shadow-md transition flex items-center gap-2">
    ...
    Agregar No. Contrato
                ...
                <button v-if="currentState >= 3 && !isArchivoActionTaken && !isArchivedView" @click="handleAction('archivo')" class="px-5 py-2.5 text-white bg-gray-600 rounded-lg hover:bg-gray-700 shadow-md transition flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Acción: Archivar Administrativamente (Visible si f_aceptado_secretaria_credito existe, se tiene contrato, se validó envío a archivo y no está archivado) -->
                <button v-if="currentState >= 7 && hasFechaAceptadoSecretariaCredito && !!numeroContrato && isArchivoActionTaken && !isArchivadoAdministrativamente && !isArchivedView" @click="handleAction('archivar-admin')" class="px-5 py-2.5 text-white bg-teal-600 rounded-lg hover:bg-teal-700 shadow-md transition flex items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" />
                    </svg>
...
const isArchivoActionTaken = computed(() => {
    ...
    return !!latest.observacion_envio;
})
...
const hasFechaAceptadoSecretariaCredito = computed(() => {
    // Check in detallesData (loaded from API)
    if (detallesData.value?.expediente?.fechas?.f_aceptado_secretaria_credito) {
        return true;
    }
    // Fallback/Early check in props (if available in list view)
    if (props.expediente?.fechas?.f_aceptado_secretaria_credito) {
        return true;
    }
    return false;
})
